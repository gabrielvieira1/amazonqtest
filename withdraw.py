...
def validate_payload(payload) -> list[str]:
    ...
    required_fields = ['iban', 'amount', 'currency']
    for field in required_fields:
        if field not in payload:
            errors.append(f'{field} is missing')
    
    # Validate amount format if present
    if 'amount' in payload and not re.match(r'^\d+\.\d{2}$', str(payload['amount'])):
        errors.append('Invalid amount')
    
    # Validate currency if present
    if 'currency' in payload and not Transaction.is_valid_currency_code(payload['currency']):
